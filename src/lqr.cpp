...
Eigen::RowVector<double, 4> LQR::get_K(
  double v_x, double yaw_des_dot, const double & tp, const size_t & index)
{
  const Eigen::Matrix4d A = get_A(v_x);
  const Eigen::Vector4d B = get_B();

  const Eigen::Vector4d C = get_C(v_x, yaw_des_dot);
  const Eigen::Matrix4d Phi = Eigen::Matrix4d::Identity() + A * tp;
  const Eigen::Vector4d Gamma = B * tp;

  RCLCPP_DEBUG(logger_, "B %f %f %f %f \n ", B(0), B(1), B(2), B(3));

  RCLCPP_DEBUG(logger_, "C %f %f %f %f \n ", C(0), C(1), C(2), C(3));

  RCLCPP_DEBUG(
    logger_, "A  %f %f %f %f \n %f %f %f %f  \n %f %f %f %f  \n %f %f %f %f  \n   ", A(0, 0),
    A(0, 1), A(0, 2), A(0, 3), A(1, 0), A(1, 1), A(1, 2), A(1, 3), A(2, 0), A(2, 1), A(2, 2),
    A(2, 3), A(3, 0), A(3, 1), A(3, 2), A(3, 3));

  RCLCPP_DEBUG(
    logger_, "Phi  %f %f %f %f  \n %f %f %f %f \n %f %f %f %f  \n %f %f %f %f  \n   ", Phi(0, 0),
    Phi(0, 1), Phi(0, 2), Phi(0, 3), Phi(1, 0), Phi(1, 1), Phi(1, 2), Phi(1, 3), Phi(2, 0),
    Phi(2, 1), Phi(2, 2), Phi(2, 3), Phi(3, 0), Phi(3, 1), Phi(3, 2), Phi(3, 3));

  RCLCPP_DEBUG(logger_, "Gamma %f %f %f %f\n ", Gamma(0), Gamma(1), Gamma(2), Gamma(3));
  ...
  RCLCPP_DEBUG(
    logger_, "P_t before %f %f %f %f  \n %f %f %f %f \n %f %f %f %f  \n %f %f %f %f  \n ", Pt(0, 0),
    Pt(0, 1), Pt(0, 2), Pt(0, 3), Pt(1, 0), Pt(1, 1), Pt(1, 2), Pt(1, 3), Pt(2, 0), Pt(2, 1),
    Pt(2, 2), Pt(2, 3), Pt(3, 0), Pt(3, 1), Pt(3, 2), Pt(3, 3));

  for (int i = 0; i < N_; i++) {
    const double res = (R_ + Gamma.transpose() * Pt * Gamma);
    const double res_loop = Gamma.transpose() * Pt * Gamma;
    RCLCPP_DEBUG(logger_, "Res loop %f ", res_loop);
  ...
  RCLCPP_DEBUG(
    logger_, "P_t after %f %f %f %f  \n %f %f %f %f \n %f %f %f %f  \n %f %f %f %f \n  ", Pt(0, 0),
    Pt(0, 1), Pt(0, 2), Pt(0, 3), Pt(1, 0), Pt(1, 1), Pt(1, 2), Pt(1, 3), Pt(2, 0), Pt(2, 1),
    Pt(2, 2), Pt(2, 3), Pt(3, 0), Pt(3, 1), Pt(3, 2), Pt(3, 3));

  const double res = (R_ + Gamma.transpose() * Pt * Gamma);
  RCLCPP_DEBUG(logger_, "Res %f ", res);
...
double LQR::calculate_control_signal(
  const double & v_x, const double & yaw_des_dot, const double & tp, const Eigen::Vector4d & xstate,
  const double & R, const size_t & index)
{
  RCLCPP_DEBUG(logger_, "Calculate control signal.");

  x_state_ = xstate;

  Eigen::RowVector<double, 4> K = get_K(v_x, yaw_des_dot, tp, index);
  RCLCPP_DEBUG(logger_, "K size %ld, %ld", K.rows(), K.cols());

  RCLCPP_DEBUG(
    logger_, "x_state_ %f %f %f %f \n ", x_state_(0), x_state_(1), x_state_(2), x_state_(3));

  RCLCPP_DEBUG(logger_, "K %f %f %f %f \n ", K(0), K(1), K(2), K(3));
  ...
  const double delta_ff = L / R + Kv * ay + K(2) * e2_ss;
